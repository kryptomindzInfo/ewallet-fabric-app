...
exports.create_ewallet = async function(req, res, _next) {
  ...
  try {
    ...
    console.log("\nSubmit Create Wallet transaction.");
    ...
    gateway.disconnect();
    console.log("\nTransaction success, send response")
    res.send({status: 1, message: "Wallet Created Successfully"});

  } catch (err) {
    //print and return error
    console.log(err);
    var error = {};
    if (err.message) { error = { status: 0, message: 'Failed to Create Wallet' } }
    if (err.endorsements) { error = { status: 0, message: err.endorsements[0].message } }
    res.send(error);
  }
};
...
exports.recharge_ewallet = async function(req, res, _next) {
  ...
  try {
    ...
    console.log("\n Submit recharge Wallet transaction ");
    if (remarks != undefined) {
    ...
    gateway.disconnect();
    console.log("\nTransaction success, send response")
    res.send({status: 1, message: "Wallet Recharged Successfully"});
  } catch (err) {
    //print and return error
    console.log(err)
    var error = {};
    if (err.message) { error = { status: 0, message: 'Failed to Recharge Wallet' }}
    if (err.endorsements) { error = { status: 0, message: err.endorsements[0].message }}
    res.send(error);
  }
};
...
exports.transfer_btw_ewallets = async function(req, res, _next) {
  ...
  try {
    ...
    if (remarks != undefined) {
      await contract.submitTransaction("transferBtwWallets", walletId1, walletId2, amount, remarks);
    } else {
      await contract.submitTransaction("transferBtwWallets", walletId1, walletId2, amount);
    }

    // Disconnect from the gateway.
    gateway.disconnect();
    console.log("\nTransaction success, send response")
    res.send({status: 1, message: "Transferred Successfully", data: {}});
  } catch (err) {
    //print and return error
    console.log(err);
    var error = {};
...
exports.show_ewallet_balance= async function(req, res, _next) {
  ...
  try {
    ...
    console.log("\nTransaction success, send response")
    res.send({status: 1, message: "Wallet Created Successfully", data: walletDetails});
  } catch (err) {
    //print and return error
    console.log(err);
    var error = {};
...
exports.get_ewallet_statement = async function(req, res, _next) {
  ...
  try {
    ...
    gateway.disconnect();
    console.log("\nTransaction success, send response")
    res.send({status: 1, message: "Wallet Created Successfully", data: statement});
  } 
  catch (err) {
    //print and return error
    console.log(err);
...
exports.get_transaction_count_of_ewallet = async function(req, res, _next) {
  ...
  try {
    ...
    // Disconnect from the gateway.
    gateway.disconnect();
    console.log("\nTransaction success, send response :", count)
    res.send({status: 1, message: "Wallet Created Successfully", data: count});
  } 
  catch (err) {
    //print and return error
    console.log(err);
